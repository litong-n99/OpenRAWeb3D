 ...
 *
 * Tests focus on: construction with height offset, contains check,
 * CandidateMapCoords sub-region, iteration in projected space,
 * and edge cases.
 */

import { describe, it, expect } from 'vitest'
import { ProjectedCellRegion } from './ProjectedCellRegion'
...
describe('ProjectedCellRegion', () => {
  ...
  describe('construction', () => {
    ...
    it('computes mapTopLeft from PPos topLeft (direct conversion)', () => {
      ...
      // mapTopLeft = PPos(5,5).toMPos() = MPos(5,5)
      expect(region.mapTopLeft.U).toBe(5)
      expect(region.mapTopLeft.V).toBe(5)
    })

    it('height offset for Rectangular is maxHeight/2', () => {
      const region = new ProjectedCellRegion(
        MapGridType.Rectangular,
        mapSize,
        10, // maxTerrainHeight = 10, offset = 10/2 = 5
        new PPos(0, 0),
        new PPos(10, 10),
      )
    ...
    it('height offset for RectangularIsometric is full maxHeight', () => {
      const region = new ProjectedCellRegion(
        ...
        mapSize,
        10, // offset = 10 (full for isometric)
        new PPos(0, 0),
        new PPos(10, 10),
      )
  ...
  describe('edge cases', () => {
    it('handles zero-height terrain correctly', () => {
      ...
      // height offset 0: mapBottomRight.V == bottomRight.V
