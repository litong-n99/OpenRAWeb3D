    ...
    try {
      ...
      switch (type) {
        ...
        case OrderType.Fields: {
          ...
          if (hasField(fields, OrderFields.ExtraActors)) {
            const raw = decoded[idx++] as number[]
            if (worldRef) {
              extraActorIds = raw.filter(
                (id) => worldRef.getActorById(id) !== undefined,
              )
            } else {
              extraActorIds = raw
            }
...
export function tryParseOrderPacket(
  ...
  // We need to split concatenated msgpack arrays. Each order is a single msgpack-encoded array.
  // We parse by repeatedly decoding from the stream.
  const serializedOrders: Uint8Array[] = []
  let offset = 0
  while (offset < remaining.length) {
    // Use msgpack decode to read one value and determine its length
    try {
      const slice = remaining.subarray(offset)
      const result = decode(slice) as unknown
      // Calculate how many bytes were consumed
      // msgpack decode returns the value; we need to track the read offset
      // NOTE: @msgpack/msgpack doesn't expose bytesRead from decode().
      // We use a heuristic: find the next msgpack array start or end of data.
      // For the common case of a single order per frame, this works.
      // For multiple orders, we need to be more careful.
      // Strategy: decode returns the value; subsequent orders start after this one.
      // We estimate by encoding the result and comparing length.
