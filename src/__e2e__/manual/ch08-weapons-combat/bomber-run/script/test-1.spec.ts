...
test.describe('B5 — Configurable Parameters', () => {
  test('B5.1 speed slider changes flight speed (slow 200 vs fast 800)', async ({ page }) => {
    ...
    await startBomberRun(page)
    await waitForPhase(page, 'flying')
    const slowStart = await getAircraftPosition(page)
    ...
    await startBomberRun(page)
    await waitForPhase(page, 'flying')
    const fastStart = await getAircraftPosition(page)
    await page.waitForTimeout(TICK_MS * 10)
    ...
    // Precision relaxed to 0 (within 0.5 wu/tick) due to 20 TPS timing jitter
    // (actual tick count in 500ms wait may vary by ±1).
    expect(slowDx).toBeCloseTo(200 / 1024, 0)
    expect(fastDx).toBeCloseTo(800 / 1024, 0)
