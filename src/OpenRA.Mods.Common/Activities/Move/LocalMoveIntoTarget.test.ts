/**
 * LocalMoveIntoTarget.test.ts — LocalMoveIntoTarget 迁移单元测试
 *
 * Tests focus on: construction, tick behavior, target line delegation.
 */

import { describe, it, expect } from 'vitest'
import { LocalMoveIntoTarget } from './LocalMoveIntoTarget.js'
...
describe('LocalMoveIntoTarget', () => {
  describe('constructor', () => {
    it('creates with target and range', () => {
      const self = mockActor()
      const target = Target.fromCell(new CPos(5, 5))
      const range = new WDist(1024)
      const move = new LocalMoveIntoTarget(self, target, range)
    ...
    it('accepts custom color', () => {
      ...
      const move = new LocalMoveIntoTarget(self, target, new WDist(1024), color)
      expect(move).toBeDefined()
    })
  })
  ...
  describe('tick', () => {
    it('completes when target is invalid', () => {
      const self = mockActor()
      const move = new LocalMoveIntoTarget(self, Target.Invalid, new WDist(1024))
    ...
    it('ticks with valid target (stub child completes immediately)', () => {
      ...
      const target = Target.fromCell(new CPos(5, 5))
      const move = new LocalMoveIntoTarget(self, target, new WDist(1024))
