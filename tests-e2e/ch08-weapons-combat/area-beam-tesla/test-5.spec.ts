...
test.describe('A5 — Beam FadeOut and Disposal', () => {
  ...
  test('A5.2 tail reaches target and beam completes', async ({ page }) => {
    // Beam completes at tick ~47 (isDestroyed fires on next simulateTick after tail arrives).
    // Use phase-based waiting instead of exact tick number for robustness.
    await page.click('#btn-fire')
    await page.waitForFunction(
      () => (window as any).__testHarness.getActiveBeam() !== null,
      {},
    ...
    // Wait for the 'done' phase (beam complete)
    await page.waitForFunction(
