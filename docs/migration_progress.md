...
> **Current phase**: 🎉🎉🎉 ALL CHAPTERS 2-27 COMPLETE (100%). ALL MIGRATION WORK COMPLETE. Content Installer ALL PHASES A+B+C COMPLETE (25/25, 100%).
> **Overall status**: 🎉🎉🎉 ALL CHAPTERS 2-27 COMPLETE: Ch2-20: 665/665 (100%) + Ch21: 54+ active files (100%, ALL PHASES A-G COMPLETE, 25 deferred + 12 legacy deferred). Chapter 22: ALL 5 PHASES COMPLETE (100%, 7 impl/test files + build script + 150+ generated assets). Content Installer: ALL PHASES A+B+C COMPLETE (25/25, 100%), ~10,000 impl lines, 500+ tests. Chapters 23-24: ALL PHASES COMPLETE (18/18, 100%). Chapter 25: ALL PHASES COMPLETE (7/7, 100%). Chapter 26: ALL PHASES A-D COMPLETE (10/10, 100% -- map loading, actor spawning, skirmish game flow, shellmap AI + ModularBot + camera, widget main menu + integration tests). Chapter 27: ALL PHASES A-E COMPLETE (15/15, 100%; commits: `58995dd`/`152890b`/`1bacfc5`/`cbb2de9`/`8ea60b1`/`e0a40a4`/`3748c28`/`f339795`/`17c8376`/`e2768d1`/`61a4f42`/`4a7b579`). Total project: ~797+ files + ~49+ content installer files, **ALL MIGRATION 100% COMPLETE** 🎉🎉🎉.
> **Planning document**: [docs/remaining_systems_migration_plan.md](docs/remaining_systems_migration_plan.md)
> **Chapter 27 Plan**: [docs/chapter27_main_menu_gui_fix_plan.md](docs/chapter27_main_menu_gui_fix_plan.md)
...
| **Chapter 23 planned files (MIX File Runtime Parsing)** | 8 (3 phases A-C, 0 new files, 3 modified), **ALL PHASES A-C COMPLETE (100%, ~850 lines, ~500 test lines, commits `da66ce4`/`5a1b685`/`1077875`)**. Plan: [chapter23_mix_runtime_plan.md](docs/chapter23_mix_runtime_plan.md) |
| **Chapter 24 planned files (Animation & 3D Visual Effects)** | 10 (4 phases A-D, 5 modified + 2 test + 3 e2e), **ALL PHASES A-D COMPLETE (10/10, 100%, 229 tests)**. Phase D commits: `55aa1b3`. Plan: [chapter24_animation_effects_plan.md](docs/chapter24_animation_effects_plan.md) |
| **Chapter 25 planned files (Shroud & Fog of War 3D)** | 7 (3 phases A-C, 2 modified + 2 test + 3 e2e), **ALL PHASES COMPLETE (7/7, 100%)**. Phase A: `fc125a9`; Phase B: `50a6676`/`a5f8de2`; Phase C: `c26e1e7`/`575c100`. Plan: [chapter25_shroud_3d_plan.md](docs/chapter25_shroud_3d_plan.md) |
| **Chapter 26 planned files (Game World & Shellmap Integration)** | 10 (4 phases A-D planned). **ALL PHASES COMPLETE (10/10, 100%)**: Phase A -- map loading & actor spawning pipeline (8 files changed, 146 tests, commits: `6901fd9`/`a1a9626`/`f589cfc`). Phase B -- skirmish game flow (1 file changed: Game.ts +446 impl, +764 test lines, 136 tests, commits: `37c5e72`/`1ccee3b`). Phase C -- shellmap AI + ModularBot + camera (7 files changed: 1 new + 5 modified + 1 new test, 20 tests, commits: `978d220`/`db5052d`). Phase D -- widget main menu + integration tests (~30 tests, commit: `f029151`). Acceptance tests: `/test/ch26-integration/map-loading/`. Plan: [chapter26_gameworld_integration_plan.md](docs/chapter26_gameworld_integration_plan.md) |
| **Chapter 27 planned files (Main Menu GUI Fix)** | 15 (5 phases A-E, 5 new + 4 modified files across scripts/, src/, public/). **🎉 ALL PHASES COMPLETE (15/15, 100%)**: Phase A -- Chrome Asset Pipeline (3 tasks, commits: `58995dd`/`152890b`): build-chrome.ts 236 lines, build-mods.ts +69 lines, 131 generated chrome JSON files + common/mod.json. Phase B -- ChromeProvider + FileSystem Wiring (3 tasks, commits: `1bacfc5`/`cbb2de9`): Game.ts `_mountModDataFolders()` extended with chrome + chromeLayout + chromeMetrics; `loadMod()` calls `ChromeProvider.initialize()`; `dispose()`/`switchMod()` call `deinitialize()`; ModData.ts JSDoc for ADR-27.3. Phase C -- Widget-Based Main Menu Activation (3 tasks, commits: `8ea60b1`/`e0a40a4`/`3748c28`): showMainMenu() wired to showMainMenuWidget() with WidgetLoader.loadWidgetTree(); 7 widget types registered; Button actions wired (Skirmish, Settings, Exit); WidgetLoader.loadUI() extended for node.id matching; 3 acceptance test pages in ch27-mainmenu/. Phase D -- Shellmap Asset Enablement (4 tasks, commits: `f339795`/`17c8376`): build-content.ts dual-output (contentModId + targetModId content.json); _onContentInstalled() 3-step pipeline (rehydrateFiles -> ChromeProvider re-init -> MapCache refresh); 4 target mod content.json generated (ra, td, d2k, ts); 3 game font files + @font-face rules + document.fonts.ready. Phase E -- Integration Testing and Visual Polish (2 tasks, commits: `e2768d1`/`61a4f42`/`4a7b579`): 3 acceptance test pages (widget-menu-rendering, button-interaction, shellmap-background); 47 integration tests in Game.mainmenu.test.ts (Widget tree loading, button onClick callbacks, Escape key, hideMainMenu cleanup, DOM overlay fallback, double-invoke protection). Plan: [chapter27_main_menu_gui_fix_plan.md](docs/chapter27_main_menu_gui_fix_plan.md) |
| **Overall project completion** | 🎉🎉🎉 ALL Chapters 2-27 COMPLETE (665/665 + 54+ active + 7 + 18 + 7 + 10 + 21 + 15 = ~797+ files, **100%**). ALL MIGRATION WORK COMPLETE. Content Installer ALL PHASES A+B+C COMPLETE (25/25 tasks, 100%, ~49 new/modified files, ~10,000 impl lines, 500+ tests). Post-migration completion phase: 52 remaining items tracked in [post_migration_completion_plan.md](docs/post_migration_completion_plan.md). |
...
> **Note**: N1-N7 replace SDL2 and OpenGL-specific platform code. N1 is item #22 in the 27-item plan. N2-N7 are beyond the 27-item plan.
...
> Note: Stub files in `OpenRA.Game/Graphics/` (~16 files: AnimationWithOffset, ChromeProvider, CursorSequence, etc.) and `OpenRA.Platforms.Default/` (~5 files: audio, font stubs) are beyond Chapter 2 scope and will be addressed in subsequent chapters.
...
|------|------|-----------|----------|-------|
| 2026-06-20 | **🎉 Ch27 Phase E Integration Testing & Visual Polish** (2 ops TODO-27.E.1-E.2, 3 test pages + 1 test file) | migration-develop | migration-review | R1 NEEDS FIXES (1 BLOCKER: core test for showMainMenuWidget Widget tree loading was not testing the correct path; 3 MAJOR: test assertions for button onClick callbacks, Escape key registration/unregistration, hideMainMenu cleanup verification; 4 MINOR: JSDoc, test naming, DOM overlay fallback edge cases, double-invoke coverage), R2 APPROVED (all fixed). 3 acceptance test pages: widget-menu-rendering, button-interaction, shellmap-background (each with index.html + main.ts + README.md). 47 integration tests in Game.mainmenu.test.ts: showMainMenuWidget Widget tree loading via WidgetLoader, button onClick handlers trigger correct Game methods, Escape key handler registration/unregistration, hideMainMenu() Widget tree cleanup, DOM overlay fallback on Widget failure, double-invoke protection. Commits: `e2768d1` (fix: shellmap-background test page 3 MINOR fixes), `61a4f42` (fix: 1 BLOCKER + 3 MAJOR + 4 MINOR), `4a7b579` (feat: 47 integration tests). CHAPTER 27 NOW 100% COMPLETE (15/15). |
| 2026-06-20 | **Ch27 Phase D Shellmap Asset Enablement** (4 ops TODO-27.D.1-D.4, 2 files changed: build-content.ts + Game.ts) | migration-develop | migration-review | R1 NEEDS FIXES (2 MAJOR: missing build step integration for content.json regeneration, font loading error boundary; 4 MINOR: JSDoc for _onContentInstalled pipeline, console warnings for missing fonts, stale TODOs), R2 APPROVED (all fixed). build-content.ts dual-output (contentModId + targetModId content.json); _onContentInstalled() 3-step pipeline (rehydrateFiles -> ChromeProvider re-init -> MapCache refresh); 4 target mod content.json generated (ra, td, d2k, ts); 3 game font files copied to public/fonts/ + @font-face rules + document.fonts.ready. Commits: `f339795` (impl), `17c8376` (review fixes). |
| 2026-06-20 | **Ch27 Phase C Widget-Based Main Menu Activation** (3 ops TODO-27.C.1-C.3, 1 file changed: Game.ts) | migration-develop | migration-review | R1 NEEDS FIXES (2 BLOCKER: empty default loadUI() call in showMainMenuWidget, missing WidgetLoader.loadUI() node.id matching; 2 MAJOR: LogicKeyListenerWidget not registered in ObjectCreator, escape key handler not connected), R2 APPROVED (all fixed). showMainMenu() calls showMainMenuWidget() with DOM fallback; showMainMenuWidget() loads from ChromeProvider via WidgetLoader.loadWidgetTree(); 7 widget types registered; button onClick handlers wired for Skirmish/Settings/Exit; WidgetLoader.loadUI() extended for node.id matching (MiniYamlParser @Name compat). Acceptance tests: 3 pages under ch27-mainmenu/ (widget-menu-rendering, button-interaction, dom-fallback). Commits: `8ea60b1` (impl), `e0a40a4` (review fixes), `3748c28` (acceptance test fixes). |
| 2026-06-20 | **Ch27 Phase B ChromeProvider + FileSystem Wiring** (3 ops TODO-27.B.1-B.3, 2 files changed: Game.ts + ModData.ts) | migration-develop | migration-review | R1 MAJOR (double-deinitialize in switchMod) + 2 MINOR (JSDoc, malformed path warn), R2 APPROVED (all fixed). Game.ts `_mountModDataFolders()` extended with chrome + chromeLayout + chromeMetrics; `loadMod()` calls `ChromeProvider.initialize()`; `dispose()`/`switchMod()` call `deinitialize()`. ModData.ts JSDoc for ADR-27.3. Commits: `1bacfc5` (impl), `cbb2de9` (review fixes). |
| 2026-06-20 | **Ch26 Phase A Map Loading & Actor Spawning Pipeline** (3 ops TODO-26.A.1-A.3, 8 files changed: 5 new + 3 modified) | migration-develop | migration-review | R1 NEEDS FIXES (1 BLOCKER, 5 MAJOR, 3 MINOR), R2 APPROVED (all fixed). New: TraitFactory.ts, ActorEntryParser.ts, ActorInitializer.ts + 3 test files (146 total tests: 80 World, 28 ActorInitializer, 19 TraitFactory, 19 ActorEntryParser). Modified: World.ts (+~520 lines), World.test.ts, ModData.ts (+~30 lines). E2E: `/test/ch26-integration/map-loading/` (5 criteria). Commits: `6901fd9` (impl), `a1a9626` (review fixes R1), `f589cfc` (e2e fixes). |
| 2026-06-20 | **Ch26 Phase B Skirmish Game Flow** (3 ops TODO-26.B.1-B.3, 1 file changed: Game.ts) | migration-develop | migration-review | R1 NEEDS FIXES (1 MAJOR, 4 MINOR), R2 APPROVED (all fixed). Game.ts (+446 lines): replaced `_showComingSoon('Skirmish')` with `_showSkirmishSetup()` DOM overlay -- map selection dropdown from MapCache, player slot Human/AI/Closed toggles, faction picker, AI difficulty selector, Start Game / Cancel buttons, `SkirmishLobbyInfo` interface. Game.test.ts (+764 lines, 136 tests): modal lifecycle, map selection, player configuration, button states, toast notification, `_collectSkirmishMaps` helper. MAJOR: missing error handling for `startGame()` failure path. MINOR: JSDoc, DOM ID consistency, toast auto-dismiss, map dedup. E2E: Not needed (DOM-based, 136 unit tests cover all interactions). Commits: `37c5e72` (impl), `1ccee3b` (review fixes R1). |
| 2026-06-20 | **Ch26 Phase C Shellmap AI + ModularBot + Camera** (2 ops TODO-26.C.1-C.2, 7 files changed: 1 new + 5 modified + 1 new test) | migration-develop | migration-review | R1 NEEDS FIXES (1 BLOCKER: missing error boundary for BotModule tick exceptions, 4 MAJOR: ModularBotPlayer deactivate cleanup, faction assignment dedup, difficulty multiplier type safety, camera target null guard, 2 MINOR), R2 APPROVED (all fixed). New: ModularBot.ts (426 lines) -- thin middleware bridging 17+ BotModule files to tick system. ModularBot.test.ts (620 lines, 20 tests): tick dispatch, BotModule delegation, player lifecycle, multi-player AI, difficulty scaling, shutdown. Modified: Game.ts (+140 lines: ModularBot creation, lifecycle, shellmap camera), Game.test.ts (+118 lines: camera lifecycle, input handler), World.ts (+24 lines: ModularBot World trait registration), 3 BotModule files (+static GetInfo interfaces). Key: Previously all 17+ BotModules were migrated but inert -- ModularBot is the first code that makes them active. E2E: Not needed (AI middleware + camera logic, all unit-testable). Commits: `978d220` (impl), `db5052d` (review fixes R1). |
| 2026-06-20 | **Ch25 Phase C Fog Visibility Trait Integration** (3 ops, ~961 lines, 80+126 impl + 148+608 test = 28 new tests) | migration-develop | migration-review | R1 NEEDS FIXES (3 MAJOR: state-change guard, pulse cleanup on uncloak, missing edge tests), R2 APPROVED (all fixed). Mesh setEnabled GPU culling in modifyRender, DetectCloaked 5-tick emissive pulse, _isDetectedByAnyEnemy enemy iteration. E2E: `/test/ch25-shroud/actor-visibility/`. Commits: `c26e1e7`, `575c100`. |
| 2026-06-20 | **Ch25 Phase B FrozenActor 3D Flash & Tint + Snapshot Capture** (2 ops, ~461 lines, 84+142 impl + 155+79 test = 9 new tests) | migration-develop | migration-review | R1 NEEDS FIXES (2 MAJOR: multiplicative 2nd cycle + alpha guard), R2 APPROVED (all fixed). Flash tint applied to material.emissiveColor/alpha via duck-typing, ReplaceColor + Multiplicative modes, savedEmissive for correct revert, blink even-ON/odd-OFF, ScreenMap add/remove for visibility transitions. E2E: `/test/ch25-shroud/frozen-actor-flash/`. Commits: `50a6676`, `a5f8de2`. |
| 2026-06-20 | **Ch25 Phase A ShroudRenderer 3D Verification & Fix** (2 ops, ~356 lines, 75+21=96 tests) | migration-develop | migration-review | APPROVED (0 BLOCKERs, 2 MAJOR doc-only, 4 MINOR resolved). ShroudRenderer now extends Component, registered as WorldActor trait in createWorldActor(), ITickRender for per-frame visibility updates, logDiagnostics() debug helper, non-square map support documented, UV axis coordinate-system documented. E2E: `/test/ch25-shroud/shroud-overlay/`. Commits: `fc125a9` (impl), follow-up (review fixes + e2e). |
| 2026-06-19 | **Content Installer Phase C (Multi-Mod Support)** (4 tasks, ~12 files changed, ~30 tests) | migration-develop | migration-review | APPROVED (R1: 2 BLOCKERs, all fixed). RSA Key Path for encrypted MIX, .tem tileset format for Tiberian Sun, C&C/D2K content verification, multi-mod switching with per-mod storage UI. ~1,500 lines. 25/25 total tasks (100%). ALL PHASES A+B+C COMPLETE. Commits: `e6757a6`, `b3206ac`, `58ffeb8`, `fc86c15`, `fe97f0b`. |
| 2026-06-19 | **Content Installer Phase B (Polish & Offline)** (8 tasks, 15 files changed, 67 tests) | migration-develop | migration-review | APPROVED (R1: 2 BLOCKER + 3 MAJOR, all fixed). Encrypted MIX (Blowfish universal key), File Upload (ContentSourceResolver), Parallel Downloads, Resume Downloads (HTTP Range), Update Check (SHA1 comparison), Storage Quota (navigator.storage.estimate), Offline Detection (navigator.onLine), Cache API (Service Worker). 3,426 lines added, 72 removed. Commits: `bbb33f2`, `2c81270`, `a25ee32`, `10d45aa`. |
...
- **NOP stubs**: 7 total — 7 SDL2/platform files, all with full documentation explaining browser/Babylon.js API alternatives.
...
## Content Installer Pipeline (ALL PHASES A+B+C COMPLETE, 25/25, 100%)
...
> **Status**: ALL PHASES A+B+C COMPLETE (25/25, 100%). Phase A: 13/13. Phase B: 8/8. Phase C: 4/4.

| Status | Count | Percentage |
|--------|-------|------------|
| Completed (Phase A) | 13 | 52% (of 25 total) |
| Completed (Phase B) | 8 | 32% |
| Completed (Phase C) | 4 | 16% |
...
| C | Multi-Mod Support | 4 (0 new + 4 modified) | COMPLETE |
...
### New Features (Phase C)

| Feature | Description |
|---------|-------------|
| RSA Key Path | Encrypted MIX uses RSA public key path for Blowfish key decryption |
| .tem Tileset Format | TemTileset parser reads `.tem` header (width, height, tile count) + tile data for Tiberian Sun |
| C&C Content Verification | Verified Tiberian Dawn content installs and game launches end-to-end |
| D2K Content Verification | Verified Dune 2000 content installs and game launches (R8/R16/RS passthrough) |
| Multi-Mod Switching | Seamless content switching between RA/C&C/D2K/TS mods with per-mod storage UI |

### Modified Modules (Phase C)

| Module | Changes |
|--------|---------|
| `scripts/build-content.ts` | Extended to generate C&C, D2K, and TS content manifests |
| `scripts/build-mixdb.ts` | Extended to include Classic and CRC32 hash databases for all four mods |
| `src/OpenRA.Mods.Cnc/FileSystem/MixFileRuntime.ts` | RSA key path + .tem tileset + encrypted MIX for TS |
| `src/OpenRA.Game/ContentInstaller/ContentInstallerService.ts` | Multi-mod switching + per-mod storage management |
