...
| **Chapter 27 planned files (Main Menu GUI Fix)** | 15 (5 phases A-E planned, 5 new + 4 modified files across scripts/, src/, public/). **Phase A COMPLETE (3/15, 20%; commits: `58995dd`/`152890b`)**: Phase A -- Chrome Asset Pipeline (3 tasks: build-chrome.ts 236 lines, build-mods.ts +69 lines, 131 generated chrome JSON files + common/mod.json). Phases B-E PLANNING (12 tasks remaining): Phase B -- ChromeProvider + FileSystem Wiring (3 tasks, Game.ts + ModData.ts). Phase C -- Widget-Based Main Menu Activation (3 tasks, Game.ts showMainMenu/showMainMenuWidget). Phase D -- Shellmap Asset Enablement (4 tasks, content.json + fonts + Game.ts). Phase E -- Integration Testing and Visual Polish (2 tasks, acceptance tests + integration tests). Plan: [chapter27_main_menu_gui_fix_plan.md](docs/chapter27_main_menu_gui_fix_plan.md) |
| **Overall project completion** | ALL Chapters 2-26 COMPLETE (665/665 + 54+ active + 7 + 18 + 7 + 10 + 21 = ~782+ files, **100%**). Chapter 27: Phase A COMPLETE (chrome asset pipeline, 3/15 tasks, 20%; commits: `58995dd`/`152890b`); Phases B-E PLANNING (12 remaining tasks). Content Installer ALL PHASES A+B+C COMPLETE (25/25 tasks, 100%, ~49 new/modified files, ~10,000 impl lines, 500+ tests). Post-migration completion phase: 52 remaining items tracked in [post_migration_completion_plan.md](docs/post_migration_completion_plan.md). |
