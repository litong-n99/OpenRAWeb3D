...
> **Chapter Status**: PLANNING (0/7 migrated)
...
|:---|:---:|:---:|:---:|:---|
| A: ShroudRenderer | 2 | ~100 | -- | PLANNING |
...
**Status**: PLANNING
**Complexity**: LOW
**Blocked by**: Nothing (ShroudRenderer code is complete, needs integration verification)
**Blocks**: Phase B (FrozenActorLayer may depend on shroud state for fog transitions)
...
- [ ] **TODO-25.A.1** `src/OpenRA.Mods.Common/Traits/World/ShroudRenderer.ts` (est. 60 lines) -- Verify and fix the 3D rendering pipeline at runtime:
...
- [ ] **TODO-25.A.2** `src/OpenRA.Mods.Common/Traits/World/ShroudRenderer.ts` (est. 40 lines) -- Ensure visibility texture maps correctly to map dimensions:
...
**Phase A Summary**: 2 operations, ~100 lines TS. After Phase A, the shroud overlay renders as a dark translucent layer over unexplored areas, with smooth edge blending between explored and unexplored cells.
...
- [ ] **TEST-25.1** ShroudRenderer: ground-plane mesh exists in scene after `worldLoaded()`
- [ ] **TEST-25.2** ShroudRenderer: visibility texture receives correct data for all-hidden initial state
- [ ] **TEST-25.3** ShroudRenderer: dirty cell tracking marks only changed cells
...
|--------|-----------|---------|
| Shroud overlay 3D | `/test/ch25-shroud/shroud-overlay/` | Verify shroud renders as dark overlay with smooth edge blending at fog boundaries |
